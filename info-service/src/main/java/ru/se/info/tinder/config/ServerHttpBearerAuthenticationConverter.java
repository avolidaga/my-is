...
public class ServerHttpBearerAuthenticationConverter implements Function<ServerWebExchange, Mono<Authentication>> {
    ...
    public Mono<Authentication> create(ResponseInfoDto responseInfoDto) {
        List<SimpleGrantedAuthority> authorities;

        authorities = responseInfoDto.getRoles().stream()
                .map(SimpleGrantedAuthority::new)
                .collect(Collectors.toList());

        return Mono.justOrEmpty(new UsernamePasswordAuthenticationToken(responseInfoDto.getUsername(), null, authorities));
